...
export async function generatePDF(
  ...
  if (totalItems > 0) {
    ...
    doc.roundedRect(25, yPosition + 12, ((pageWidth - 50) * progress) / 100, 4, 2, 2, 'F');
  }
  ...
  // Save the PDF
  const fileName = `packing-list-${tripDetails.substring(0, 30).replace(/[^a-z0-9]/gi, '-').toLowerCase()}-${Date.now()}.pdf`;
  doc.save(fileName);
